...
def test_inventory_page_loads(driver):
    ...
    assert inventory_page.get_page_title() == "Products", "Título de página de inventario incorrecto"
...
def test_sort_inventory(driver, sort_option, expected_order):
    ...
    if "Name" in sort_option:
        names = [item.find_element_by_class_name("inventory_item_name").text for item in items]
        assert names == expected_order(names), f"Ítems no ordenados por {sort_option}"
    else:
        prices = [float(item.find_element_by_class_name("inventory_item_price").text.replace("$", "")) for item in items]
